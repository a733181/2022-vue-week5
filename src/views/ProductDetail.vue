<template>
  <div class="grid lg:grid-cols-2 gap-7">
    <div class="w-full">
      ...
      />
      <ul class="flex">
        <li class="img" :class="activeImg(product.imageUrl)" @click="cacheImage = product.imageUrl">
    ...
    <section class="flex flex-col mt-6">
      ...
      <div class="flex items-center my-6 lg:mt-auto lg:mb-[72px]">
        ...
        <base-btn class="w-full" @click="addCart">加入購物車</base-btn>
      </div>
    </section>
  </div>
  <div class="py-2 mt-3 border-t-2">
    <h2>注意事項</h2>
  ...
  <base-dialog :show="!!addCartMessage" title="已加入購物車" @close="closeAddCartMessage">
    <add-cart-product :product="product" :quantity="productQuantity"></add-cart-product>
  </base-dialog>
</template>
...
export default {
  ...
  methods: {
    async getProductDetail() {
      try {
        this.isLoading = true;
        await this.$store.dispatch('product/getProductDetails', this.id);
        this.product = await this.$store.getters['product/productData'];
